...
### 🔄 Обновление данных
- В текущей версии данные обновляются при нажатии кнопки "Обновить"
- В будущих версиях планируется автоматическое обновление через WebSocket:
...
В текущей версии MVP мы намеренно ограничили количество индексируемых блоков до 8 последних блоков. Это решение принято по следующим причинам:
...
const BLOCKS_LIMIT = 8; // Ограничение на количество блоков
...
processor.addBlockHandler({
  async process(ctx) {
    ...
    // Очищаем старые блоки, оставляя только последние 8
